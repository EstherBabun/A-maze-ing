...
class MazeRenderer:
    ...
    COLOR_BG = 0x1A1A1A 
    COLOR_42 = 0x00CC00
    WALL_THICKNESS = 1  # Fixed typo
    CELL_SIZE = 30  # Default, will be overridden
    ...
    def __init__(self, output_file: str) -> None:
        ...
        if ret != 0:
            screen_width, screen_height = 1920, 1080  # Fixed variable names
        ...
        if maze_h > maze_w:
            ...
            # Available space for maze image = usable_width / 1.60
        ...
        # Calculate window size with extra UI space
        if maze_h > maze_w:
    ...
    def extract_info(self, filename: str) -> tuple:  # Fixed: added self, renamed method
        """Extract the content of the output file."""
        try:
            with open(filename, "r") as f:
                lines = [line.rstrip("\n") for line in f.readlines()]  # Fixed: line not lines
        except (FileNotFoundError, PermissionError) as e:
            print(f"Error: {e}")
            return ([], 0, 0)  # Return proper tuple with defaults
